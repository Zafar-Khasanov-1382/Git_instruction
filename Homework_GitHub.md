...
4. При помощи команды **git pull** вытянуть/cкачать актуальное состояние из удалённого репозитория с последующим слиянием веток. **git pull** является составной командой, состоящей из **git fetch** _(получить)_ и **git merge** _(слить)_. При возникновении конфликта, последний решается также, как и при **git merge**.